...
/*! define method.
  ...
  @return defined mrbc_method.
*/
#ifdef ESP32_COPRO
static mrbc_method * impl_mrbc_define_method(struct VM *vm, mrbc_class *cls, const char *name, mrbc_func_t cfunc)
#else
...
void mrbc_define_method(struct VM *vm, mrbc_class *cls, const char *name, mrbc_func_t cfunc)
{
  mrbc_method * m = impl_mrbc_define_method(vm, cls, name, cfunc);
  m->copro_func = NULL;
}
...
/*! define method.
  ...
  @param  cfunc		pointer to function.
*/
void mrbc_define_method_copro(struct VM *vm, mrbc_class *cls, const char *name, mrbc_func_t cfunc, void * coprofunc)
{
  mrbc_method * m = impl_mrbc_define_method(vm, cls, name, cfunc);
  m->copro_func = coprofunc;
}
#endif
  ...
  while( 1 ) {
	...
	*r_method = *method;
    ...
    if( c->method_symbols[right] == sym_id ) {
      ...
      r_method->cls = cls;
#if ESP32_COPRO
      r_method->copro_func = c->method_copros == NULL ? NULL : c->method_copros[right];
#endif
      return r_method;
    }
